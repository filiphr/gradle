    ...
    private void notifyAfterEvaluate(ProjectEvaluationListener listener, ProjectInternal project, ProjectStateInternal state) {
        try {
            ...
            if (state.hasFailure()) {
                ...
                String infoMessage = "Project evaluation failed, leading to a follow-up error in Project.afterEvaluate {}.";
                if (logStackTraces) {
                    LOGGER.error(infoMessage, e);
                } else {
                    LOGGER.error(infoMessage + " Run with --stacktrace to see the follow-up error.");
                }
                return;
            }
            addConfigurationFailure(project, state, e);
